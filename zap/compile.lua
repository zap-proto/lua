...
-- cjson is only used for optional debug logging (see dbgf below); load it
-- lazily so the compiler is usable under a bare LuaJIT without the C module.
local function cjson()
...
function comp_header(res, nodes)
    dbg("compile_headers")
    insert(res, format([[
-- Generated by lua-zap %s on %s
-- https://github.com/zap-proto/lua
...
function process_annotations(annos, nodes)
    if debug then
        dbg("process_annotations:" .. cjson().encode(annos))
    end
